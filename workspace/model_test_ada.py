...
from utils.direction import dir2pos, sample_dir
...
from workspace.GPT_Baseline.gpt import VisionBaseline
...
def random_sampled_points(pcs):
    ...
    result = sample_dir(point[:3], point[3:6])
...
def sample_hand_pose(network, pcs, cp1, interaction, points_num=2048, rvs=100, show=False):
    ...
    with torch.no_grad():
        ...
        hand_pos, hand_ori = dir2pos(sampled_point[:3], action_direct)

    return hand_pos, hand_ori, sampled_point, action_direct


def sample_hand_pose_multi(network, pcs, cp1, interaction, points_num=2048, rvs=50, show=False):
    aff = network["aff"]
    actor = network["actor"]
    critic = network["critic"]
    device = network["device"]
    ...
    print("sample time:{:.6f}:".format(end-start))
    return hand_pos, hand_ori, sampled_point, action_direct


def llm_to_action(env, baseline: VisionBaseline, task_name: str, object_name: str):
    """
    1. Gets a 2D action point from the VLM.
    2. Converts the entire depth map to a point cloud to get points and normals.
    3. Finds the specific 3D point and normal corresponding to the 2D point.
    4. Generates a full action tuple (hand_pos, hand_ori, point, action_dir).
    """
    print("Getting VLM observation images from the environment...")
    rgb_images_list, depth_images_list = env.get_vlm_observation_images(0)

    print("Querying VLM for a 2D action point...")
    action_point_2d = baseline.get_action_point(rgb_images_list, task_name, object_name,show=True, heuristic=False)

    if action_point_2d is None:
        print("LLM baseline failed to return a 2D point.")
        return None

    img_idx, pixel_x, pixel_y = action_point_2d
    
    if img_idx < 0 or img_idx >= len(depth_images_list):
        print(f"Error: Image index {img_idx} is out of bounds for the depth images list.")
        return None

    cam_list=["front", "back", "left", "right"]
    
    # --- depth_image_to_point_cloud_GPU ---
    print(
        f"Generating full point cloud for the front view to find point at {cam_list[img_idx]}: ({pixel_x}, {pixel_y})..."
    )
    front_depth_map = torch.tensor(depth_images_list[img_idx], device=env.device)
    
    front_cam_matrices = env.camera_matix[0][cam_list[img_idx]]

    img_h, img_w = front_depth_map.shape
    if not (0 <= pixel_y < img_h and 0 <= pixel_x < img_w):
        print(
            f"Error: 2D point {action_point_2d} is outside of image dimensions ({img_w}x{img_h})."
        )
        return None

    points, normals = depth_image_to_point_cloud_GPU(
        depth_image=front_depth_map,
        camera_view_matrix=torch.tensor(front_cam_matrices["view"], device=env.device),
        camera_proj_matrix=torch.tensor(front_cam_matrices["proj"], device=env.device),
        width=env.img_size[0],
        height=env.img_size[1],
        device=env.device,
    )

    index = pixel_y * env.img_size[0] + pixel_x

    point_3d = points[index]
    normal_3d = normals[index]

    point_6d = torch.cat((point_3d, normal_3d))

    print(
        f"Found 3D point: {point_3d.cpu().numpy()} with normal: {normal_3d.cpu().numpy()}"
    )

    print("Generating hand pose from the 3D point and normal...")
    result = sample_dir(point_6d[:3], point_6d[3:6])

    if result is False:
        print("Failed to generate a valid hand pose from the selected point.")
        return None

    hand_pos, hand_ori, action_dir = result
    print("Successfully generated action tuple.")

    return hand_pos, hand_ori, point_6d, action_dir
...
def collect_data(task_config, network, collect=False, test_model="model",without_interaction=False):
    ...
    parts_num= len(task_config["part_names"])-2

    if test_model == "gpt":
        baseline=VisionBaseline("/home/user/Dual-Assemble/workspace/GPT_Baseline/prompts","/home/user/Dual-Assemble/workspace/GPT_Baseline/api_key.json","qwen-vl-plus")
        list1=["bucket","drawer_c","cask"]
        list2=["desk_1_pull", "desk_2_pull", "desk_3_pull", "drawer_top_1_r", "drawer_top_2_r", "drawer_top_r", "drawer_bottom_r", "drawer_bottom_1_r", "drawer_bottom_2_r"]
        list3=["drawer","cabinet"]
        list4=["chair","desk","table","lamp"]
        if any(tn in task_name for tn in list1):
            gpt_task_name = "pick_up"
        elif any(tn in task_name for tn in list2):
            gpt_task_name = "pull"
        elif any(tn in task_name for tn in list3):
            gpt_task_name = "push"
        elif any(tn in task_name for tn in list4):
            gpt_task_name = "screw"
        gpt_obj_name= task_config["part_names"][0]
    ...
    for collect_count in range(10000000):
        ...
        # cur_step = len(cur_obs) - 1
        
        result = obs_to_pc_fps(cur_obs, env.camera_matix[0], env.img_size, [1, 2]+list(range(3, parts_num + 1)), npoint=10240 if save_ply else 2048, show=save_ply)
        if result is False:
        ...
        # part3_points=points[3]
        # part3_normals=normals[3]
        # part4_points=points[4]
        # part4_normals=normals[4]
        # part1_points =torch.cat((part1_points,part3_points,part4_points),dim=0)
        # part1_normals =torch.cat((part1_normals,part3_normals,part4_normals),dim=0)
        for i in range(3, parts_num + 1):
            o_part_points = points[i]
            o_part_normals = normals[i]
            part1_points = torch.cat((part1_points, o_part_points), dim=0)
            part1_normals = torch.cat((part1_normals, o_part_normals), dim=0)

        pcs1 = torch.cat((part1_points, part1_normals), dim=1)
        pcs2 = torch.cat((part2_points, part2_normals), dim=1)
        pcs = torch.cat((pcs1, pcs2), dim=0)
        ...
        dist = torch.norm(part2_points - ee_pos, dim=1)
        ...
        if test_model == "model_single":
            hand_pos, hand_ori, sampled_point, action_direct = sample_hand_pose(
                network, pcs, cp1, interaction_p,points_num=part1_points.shape[0]
            )
        elif test_model == "model":
            hand_pos, hand_ori, sampled_point, action_direct = sample_hand_pose_multi(
                network, pcs, cp1, interaction_p,points_num=None if save_ply else part1_points.shape[0], show=save_ply
            )
        elif test_model == "random":
            result = random_sampled_points(pcs1)
            if result is False:
                print("Random Sample Failed")
                continue
            hand_pos, hand_ori, sampled_point, action_direct = result
        elif test_model == "heuristic":
            result = heuristic_sample(pcs1, start_part1_pose, task_name)
            if result is False:
                print("Random Sample Failed")
                continue
            hand_pos, hand_ori, sampled_point, action_direct = result
            ...
            result = llm_to_action(env, baseline, gpt_task_name, gpt_obj_name)
            if result is None:
                print("LLM Baseline failed to return a valid action.")
                continue
            hand_pos, hand_ori, sampled_point, action_direct = result
        else:
        ...
        hand_pos -= 0.008 * action_direct.cpu().numpy()
        ...
        if not contact_flag:
            ...
            env.set_check(True)
            if test_model == "random" or test_model == "heuristic" or test_model == "gpt":
                env.set_check(False)
            ...
            while env.need_reset:
                ...
                result = obs_to_pc_fps(
                    cur_obs, env.camera_matix[0], env.img_size, [1, 2]+list(range(3,parts_num+1)),npoint=10240 if save_ply else 2048, show=save_ply
                )
                ...
                if test_model == "model_single":
                    hand_pos, hand_ori, sampled_point, action_direct = sample_hand_pose(
                        network, pcs, cp1, interaction,points_num=part1_points.shape[0]
                    )
                elif test_model == "model":
                    hand_pos, hand_ori, sampled_point, action_direct = (
                        sample_hand_pose_multi(network, pcs, cp1, interaction,points_num=None if save_ply else part1_points.shape[0], show=save_ply)
                    )
                elif test_model == "random":
                    result = random_sampled_points(pcs1)
                    if result is False:
            ...
            if all_finished:
                ...
                if abs(final_z - start_z) > 0.1:
                    print("part1 out of table")
                    continue
            ...
            write_data = np.array([[100.0, 0, 0, 0, 100.0, 100.0, 100.0]])
            file_exist = os.path.exists(os.path.join(base_data_path, "log.csv"))
            with open(os.path.join(base_data_path, "log.csv"), "a") as f:
                np.savetxt(
                    f,
...
if __name__ == "__main__":
    ...
    argparse.add_argument("--aff_ckpt", type=str, help="affordance checkpoint path")
    argparse.add_argument("--actor_ckpt", type=str, help="actor checkpoint path")
    argparse.add_argument("--critic_ckpt", type=str, help="critic checkpoint path")
    argparse.add_argument("--test_model",type=str,default="model",help="test model type: random|model_single|model|heuristic",)
    argparse.add_argument("--without_interaction",action="store_true",help="whether to use interaction or not, only for model_single and model",)
    args = argparse.parse_args()
    task_name = args.task
    test_model = args.test_model
    without_interaction = args.without_interaction
    task_config = all_task_config[task_name]
    # random|model_single|model|Heuristic
    print(f"Task: {task_name} | Test Model: {test_model}")
    config = {
        "aff_ckpt": args.aff_ckpt,

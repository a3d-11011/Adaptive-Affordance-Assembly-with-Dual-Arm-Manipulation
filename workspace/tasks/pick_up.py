...
def reset_global(task_n):
    global task_name, task_config, furniture_name, part1_name
    task_name = task_n
    task_config = all_task_config[task_name]
    furniture_name = task_config["furniture_name"]
    part1_name = task_config["part_names"][0]
...
def pre_grasp(env, start_ee_states, env_states):
    ...
    target_pos_1 = part1_pos
    target_pos_1[1] += 0.191 * np.cos(part1_forward) * factor
    target_pos_1[0] -= 0.191 * np.sin(part1_forward) * factor
    target_pos_1[1] += last_length * np.sin(part1_forward) * factor
    target_pos_1[0] += last_length * np.cos(part1_forward) * factor
    target_pos_1[2] += 0.19 * factor
...
def lift_down(env, start_ee_states, env_states):
    start_pos_1, start_quat_1, gripper_1 = start_ee_states[0]
    part1_pose = env_states[0]
    now_z = part1_pose[2, 3]
    reset_pose = find_part1_reset_pose(env)
    reset_z = abs(reset_pose[2])

    need_z = now_z - reset_z
    # print("Need Z:", need_z)
    target_pos_1 = start_pos_1
    target_pos_1[2] -= need_z
    target_ee_states = [(target_pos_1, start_quat_1, gripper_1)]
    thresholds = [(None, None), (None, None)]
    # hand_pose=env.franka_hand_pose
    # p=hand_pose.p
    # r=hand_pose.r
    # target_hand_pos=np.array([p.x,p.y,p.z-need_z.cpu()])
    # cur_q=np.array([r.x,r.y,r.z,r.w])
    # set_hand_transform_with_rt(env, target_hand_pos,cur_q)
    return target_ee_states, thresholds, False


def find_part1_reset_pose(env):
    from furniture_bench.assemble_config import config
    from furniture_bench.utils.pose import get_mat

    reset = config["furniture"][env.furniture_name][part1_name]["reset_pos"][0]
    reset = torch.tensor(reset).cpu()
    # print("Reset Pose:", reset)
    scaled = reset * env.furniture_scale_factor
...
def reset_random_length():
    global last_length
    last_length = np.random.uniform(-0.15, 0.15)
    if task_name == "cask":
        last_length = 0.0
...
def perform_r(env, prev_target_ee_states):
    target_ee_states = prev_target_ee_states
    # for i in range(2):
    target_ee_states, result = act_phase(env, "lift_down", func_map, target_ee_states)
    return True
